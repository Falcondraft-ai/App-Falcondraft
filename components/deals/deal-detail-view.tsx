...
type DealDetailData = Awaited<ReturnType<typeof getDealDetail>>;
type DealRecord = NonNullable<DealDetailData["deal"]>;

/**
 * Shared deal/proposal detail screen, rendered both by the sales dashboard
 * (/dashboard/deals/[id]) and the broker SaaS proposal module
 * (/courtier/deals/[id]). The two surfaces only differ by their breadcrumb
 * back-link, supplied via `backHref` / `backLabel`; every nested action
 * component resolves its own links through the basePath context.
 */
export function DealDetailView({
  ...
  deal: DealRecord;
  activity: DealDetailData["activity"];
  documents: DealDetailData["documents"];
  linkedTranscript: Awaited<ReturnType<typeof getLinkedTranscriptForDeal>>;
  availableTranscripts: Awaited<ReturnType<typeof getTranscriptsForLinking>>;
  canEdit: boolean;
  backHref: string;
  backLabel: string;
}) {
  const quoteDocument = documents.find(
    (document) =>
      document.type === "quote_pdf" || document.type === "billing_quote",
  );
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <PageBreadcrumb parent={backLabel} parentHref={backHref} current={deal.name} />
        <header
