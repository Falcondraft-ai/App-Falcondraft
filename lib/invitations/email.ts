...
const logoContentId = "falcondraft-logo";
...
function getLogoAttachment() {
  ...
  try {
    cachedLogo = readFileSync(
      join(process.cwd(), "public/falcondraft-logo.png"),
    );
  } catch {
    cachedLogo = null;
  }
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:580px;background:#ffffff;border:1px solid #dfe4ea;border-radius:20px;border-collapse:separate;overflow:hidden;">
                <tr>
                  <td align="center" style="padding:42px 38px 22px;">
                    <img src="cid:${logoContentId}" width="178" alt="FalconDraft" style="display:block;width:178px;max-width:72%;height:auto;border:0;outline:none;text-decoration:none;">
...
function buildInvitationHtmlFallback(input: InvitationEmailInput) {
  const organizationName = escapeHtml(input.organizationName);
  const roleLabel = escapeHtml(input.roleLabel);
  const acceptUrl = escapeHtml(input.acceptUrl);
          ...
          <tr>
            <td align="center" style="padding:44px 16px 34px;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:580px;background:#ffffff;border:1px solid #dfe4ea;border-radius:20px;border-collapse:separate;overflow:hidden;">
                <tr>
                  <td align="center" style="padding:42px 38px 22px;">
                    <div style="color:#0d223d;font-size:28px;font-weight:800;letter-spacing:-0.04em;">FalconDraft</div>
                    <div style="margin-top:4px;color:#5d6878;font-size:12px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;">Propositions commerciales</div>
                    <div style="width:56px;height:2px;background:#d59436;margin:28px auto 0;line-height:2px;font-size:2px;">&nbsp;</div>
                  </td>
                </tr>
        <tr>
                  <td style="padding:0 38px 38px;">
                    <h1 style="margin:0 0 22px;color:#0d223d;font-size:26px;line-height:1.24;font-weight:800;letter-spacing:-0.02em;">Invitation à rejoindre un espace FalconDraft</h1>
                    <p style="margin:0 0 18px;color:#344156;font-size:15px;line-height:1.72;">Bonjour,</p>
                    <p style="margin:0 0 18px;color:#344156;font-size:15px;line-height:1.72;">Vous avez été invité à rejoindre l’espace <strong style="color:#0d223d;font-weight:700;">${organizationName}</strong> sur FalconDraft.</p>
                    <p style="margin:0 0 18px;color:#344156;font-size:15px;line-height:1.72;">FalconDraft permet à votre équipe de centraliser ses dossiers commerciaux, générer ses propositions et suivre ses documents depuis un espace sécurisé.</p>
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:24px 0;border-collapse:collapse;">
                      <tr>
                        <td style="padding:15px 17px;background:#f7f8fa;border:1px solid #dfe4ea;border-radius:12px;color:#5d6878;font-size:14px;line-height:1.55;">
                          Rôle attribué&nbsp;: <strong style="color:#0d223d;font-weight:700;">${roleLabel}</strong><br>
                    ...
                    <p style="margin:0 0 26px;color:#344156;font-size:15px;line-height:1.72;">Pour accepter l’invitation et créer votre accès, cliquez sur le bouton ci-dessous.</p>
                    <table role="presentation" cellspacing="0" cellpadding="0" style="margin:0 auto 28px;border-collapse:collapse;">
                      <tr>
                        <td align="center" bgcolor="#0d223d" style="border-radius:11px;">
                          <a href="${acceptUrl}" style="display:inline-block;padding:15px 27px;color:#ffffff;background:#0d223d;border-radius:11px;font-size:15px;font-weight:700;text-decoration:none;">Accepter l’invitation</a>
                        </td>
                      </tr>
                    </table>
                    <div style="margin:0 0 28px;padding:16px 18px;background:#f7f8fa;border:1px solid #dfe4ea;border-radius:12px;">
                      <p style="margin:0 0 8px;color:#5d6878;font-size:13px;line-height:1.6;">Si le bouton ne fonctionne pas, copiez ce lien dans votre navigateur&nbsp;:</p>
                      <p style="margin:0;color:#0d223d;font-size:13px;line-height:1.6;word-break:break-all;"><a href="${acceptUrl}" style="color:#0d223d;text-decoration:underline;text-underline-offset:3px;">${acceptUrl}</a></p>
                    </div>
                    <p style="margin:0 0 26px;color:#5d6878;font-size:13px;line-height:1.7;">Si vous n’êtes pas à l’origine de cette demande ou si vous ne connaissez pas cette organisation, vous pouvez ignorer cet email.</p>
                    <p style="margin:0;color:#102033;font-size:15px;line-height:1.7;">Bien cordialement,<br><strong style="font-weight:800;color:#0d223d;">L’équipe FalconDraft</strong></p>
                  </td>
                </tr>
              </table>
              <p style="margin:22px 0 0;color:#8a94a3;font-size:12px;line-height:1.6;">Cet email a été envoyé automatiquement par FalconDraft. Merci de ne pas y répondre directement.</p>
            </td>
          </tr>
        </table>
      </body>
...
export async function sendInvitationEmail(input: InvitationEmailInput) {
  ...
  const { error } = await resend.emails.send({
    ...
    html: logo
      ? buildInvitationHtml(input)
      : buildInvitationHtmlFallback(input),
    attachments: logo
      ? [
          {
            content: logo,
            filename: "falcondraft-logo.png",
