...
async function insertDraftAuditLog(
  ...
  if (error) {
    console.warn("Email draft audit log insert failed.", {
...
function statusForDraftError(error: unknown) {
  ...
  if (error.message.includes("Utilisateur non autorisé")) {
    return 403;
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!isValidSecret(getRequestSecret(request), configuredSecret)) {
    console.warn("Rejected unauthorized n8n Gmail draft request.");
  ...
  if (!parsedBody.success) {
    ...
    console.warn("Rejected invalid n8n Gmail draft payload.", {
  ...
  if (membershipError || !membership) {
    ...
    console.warn("Rejected n8n Gmail draft request for inactive member.", {
      ...logContext,
      reason: membershipError?.message ?? "active_membership_missing",
    });
    return jsonError(
  ...
  if (dealId) {
    ...
    if (dealError || !deal) {
      ...
      console.warn("Rejected n8n Gmail draft request for missing deal.", {
        ...logContext,
        reason: dealError?.message ?? "deal_not_found",
      });
      return jsonError(
  ...
  try {
    ...
    console.warn("Rejected invalid n8n Gmail draft PDF attachment.", {
      ...logContext,
      reason: error instanceof Error ? error.message : "invalid_attachment",
    });
